    ...
    <div class="DetailsBeautyDivisionC">
      <div class="DetailsBeautyDivisionPadding">
        <div class="DetailsBeautyDivisionHead">
          ...
          <div>
            <div class="isCollectsBox">
              <div @click="isShouC" :class="[is_collect?'isCollects':'isCollectsEqs']">{{ is_collect?'已收藏':'收藏' }}</div>
            </div>
            <div class="DetailsBeautyDivisionBtn" v-if="sid != '0'" @click="liJiBtn()">立即预约</div>
          </div>
        ...
        <div class="DetailsBeautyDivisionCont">
          ...
          <div class="DetailsBeautyDivisionContTitle update">
            <div class="DetailsBeautyDivisionContTitleIcon"></div>
            <div class="DetailsBeautyDivisionContTitleTxt">美丽印象</div>
            <div class="checkMore" v-on:click="goStaffEvaluate">
              <span class="mr7">查看全部评价</span>
              <van-icon name="arrow"/>
            </div>
          </div>
          ...
          <div class="evaluate" v-for="item in discusslist">
            <p class="evaluate-name">{{item.name}}</p>
            <p class="evaluate-content">{{item.member_content}}</p>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      goStaffEvaluate() {//美丽印象
        this.$router.push({name: 'staffEvaluate', params: {id: this.id}})
      },
      isShouC() {
        this.$request({
